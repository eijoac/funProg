...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match {
    case List() => List()
    case x :: xs => insertFreq(x, times(xs))
  }
  
  def insertFreq(char: Char, freqs: List[(Char, Int)]): List[(Char, Int)] = {
    if (freqs.isEmpty) List((char, 1))
    else if (char == freqs.head._1) (freqs.head._1, freqs.head._2 + 1) :: freqs.tail
    else freqs.head :: insertFreq(char, freqs.tail)
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case List() => List()
    case y :: ys => insertOrder(y, makeOrderedLeafList(ys))
  }

  def insertOrder(freq: (Char, Int), leaves: List[Leaf]) : List[Leaf] = leaves match {
    case List() => List(Leaf(freq._1, freq._2))
    case y :: ys => if (freq._2 <= y.weight) Leaf(freq._1, freq._2) :: leaves else y :: insertOrder(freq, ys) 
  }
  ...
  */
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = 
    decodeHelper(tree, tree, bits, List[Char]())

    def decodeHelper(tree: CodeTree, subTree: CodeTree, bits: List[Bit], msg: List[Char]): List[Char] = {
      if (bits.isEmpty) subTree match {
        case(x: Fork) => throw new Error("decode: this cannot be the case")
        case(x: Leaf) => msg ::: List(x.char)
      }
      else subTree match {
        case (x: Fork) => 
          if (bits.head == 0) decodeHelper(tree, x.left, bits.tail, msg)
          else decodeHelper(tree, x.right, bits.tail, msg)
        case (x: Leaf) => decodeHelper(tree, tree, bits, msg ::: List(x.char))
      } 
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = 
    ...
    else encoderHelper(tree, text.head, List[Bit]()) ::: encode(tree)(text.tail)

  def encoderHelper(tree: CodeTree, char: Char, code: List[Bit]): List[Bit] = 
    tree match {
      case (x: Leaf) => 
        if (x.char == char) code
        else List[Bit]()
   ...
   */
  // alternatively, table.find(_._1 == char).get._2
  // http://stackoverflow.com/questions/16206414/scala-finding-a-specific-tuple-in-a-list
