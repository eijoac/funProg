...
class AnagramsSuite extends FunSuite  {
  ...
  test("sentence anagrams: my") {
    val sentence = List("my")
    val anas = List(
      List("my")
    )
    assert(sentenceAnagrams(sentence).toSet === anas.toSet)
  ...
  test("sentence anagrams: an") {
    val sentence = List("an")
    val anas = List(
      List("an")
    )
    assert(sentenceAnagrams(sentence).toSet === anas.toSet)
  ...
  test("dictoary lookup") {
    println(sentenceAnagrams(List()))
    assert(dictionaryByOccurrences(List(('m', 1), ('y', 1))) === List("my"))
  }
